...
public static class SECTR_StreamExport
{
	...
	public static bool ExportToChunk(SECTR_Sector sector)
	{
		...
		// Dynamic and extra-large objects need to stay in the master scene.
		foreach(SECTR_Member member in allMembers)
		{
			if(member.transform.parent == sector.transform && member.Sectors.Count > 1)
			{
	...
	public static void DeleteExportedSector(SECTR_Sector sector)
	{
		...
		// Dynamic and extra-large objects need to stay in the master scene, so unparent them.
